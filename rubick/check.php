<?php
    ...
    $uid = $_SESSION['id'];
        ...
        $s = "SELECT * FROM rtoken order by token_num DESC LIMIT 1";
        $r = mysqli_query($conn,$s);
        $n = mysqli_fetch_assoc($r);
        $n1 = $n['token_num']+1;
    
        $empty = "SELECT * FROM rtoken";
        ...
        if($num_of_ppl == 0){
            //no users found
            $d = time();
            $sql = "INSERT INTO rtoken VALUES ('$uid','$d','$n1')";
        ...
        else if($num_of_ppl > 0)
        {
            ...
            $sql = "SELECT * FROM rtoken WHERE u_id IN ('$uid')";
            $result = mysqli_query($conn,$sql);
            ...
            if($row == 0){
                ...
                $sql2 = "SELECT * FROM rtoken ORDER BY ttoken ASC";
